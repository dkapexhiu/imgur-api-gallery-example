...
const FullPost = ({ post }) => {
  return (
    <div className="flex w-full h-full gap-16 justify-center text-white font-normal">
      <div className="my-auto absolute bottom-10 flex md:flex flex-row md:flex-col items-center justify-center md:relative gap-2 px-3 py-2 md-py-5 border bg-red-300 md:bg-transparent border-yellow-400 rounded-md custom-scroll-bar">
        {post?.score != null && (
      ...
      <div className="flex-col sm:w-90 md:w-[auto] overflow-y-scroll gap-2">
        <div className="text-xl mb-3">{post?.title ?? ''}</div>
        {post?.images?.map((img, index) => {
          return (
            <div key={index}>
              <img src={img?.link} alt={`Post Image ${index + 1}`} className="w-100 mb-2" />
            </div>
          );
        })}
      </div>
